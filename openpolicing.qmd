...
execute:
  echo: false
description: Some SQL exploration of the Stanford Open Policing Project, on lighter and more serious topics. 
...
#| echo: true
#| fig-alt: "A 2 x 10 table that shows the top ten most frequent vehicle registration states for arrests made in Seattle, WA." 
...
```{sql connection=con_traffic}
#| echo: true
#| fig-alt: "A 2 x 3 table displaying the number of arrests where the plate was either Washington, not-Washington, or unknown."
...
Not really. There are still way more plates that go unrecorded than plates that do.

However: Seattle is a bit unique about plates,and if you've lived there for a while, you'll pretty quickly notice what's up. In 2009, the DOL switched to a 7-character plate system (CCC-####) and at first, EVERY plate started with an A. Then around 2015, when they ran out of unique combinations, a B. By sometime after 2020, newly issued plates started with a C.

Not so helpful when trying to identify a car, because if you only remember the first letter of a plate, it's no help at all. But, it does mean that ALL Washington licence plates start with an A, B, or C (since 2009) (unless they are customized). This probably means that Seattle PD is only recording "WA" when they feel like it's necessary; eg, when the plate number is really old or customized. This would help explain the sheer number of NULL values. Ignoring customizations, any plate that starts with an A, B, or C and fits the CCC-#### system is going to be a Washington plate, so there's no need to write that down – and any OTHER standard plate is an out-of-towner.
...
Unfortunately, both the statewide Illinois data table and the Chicago data table have no row for vehicle registration state, so I can't compare this trend to Illinois.

## Stops, Indigenous Americans, and Age

Next I was exploring which police departments had more possible entries for subject race than others. North Dakota, for example, had the standard lineup (White, Asian, African American, Hispanic, Other) but with the added distinction of one group that is lumped into "Other" in many other states: Native American. I decided to look at the ages of traffic stops made on Native Americans, as I had noticed that many violations were for driving without a licence.

```{sql connection=con_traffic}
#| echo: true
#| fig-alt: "The average age of subjects in traffic stops in North Dakota, approximately 36.1."
...
```{sql connection=con_traffic}
#| echo: true
#| fig-alt: "The average age of Native American subjects in traffic stops in North Dakota, approximately 33.7."
...
```{sql connection=con_traffic}
#| echo: true
#| fig-alt: "The percent of Native Americans in traffic stops who are minors and the percent of non-Native Americans in traffic stops who are minors, approximately 3.7% and 3.1% respectively."
...
Not only is the average age of Native Americans who get stopped in North Dakota lower than that of the total average, but there is a notable increase in Native Americans minors who get stopped. Most other data tables, including ones for areas with very high Native populations, do not list Native American as an option (I assume they are grouped under 'other', but as I cannot sort them, I cannot say for sure.) Upon looking at Tulsa, Oklahoma, which is known to have a high Indigenous population, I found the abbreviated race 'I' (which I believe stands for Indian in this case, as many official state and tribal documents in Oklahoma still use this language).

Unfortunately, as I dug through the Tulsa, OK table, I realized that subject age was not recorded. But I did find something else interesting, which is that the Tulsa, Oklahoma PD also has a lot of non-standard race abbreviations. 

```{sql connection=con_traffic}
#| echo: true
#| fig-alt: "A list of every unique race abbreviation recorded by the Tulsa, OK PD."
